...
export default function MemberBikeRankMonth() {
	const [weekrank, setweekrank] = useState([]);
    const [userrank, setuserrank] = useState();
	const [userdist, setuserdist] = useState();
    const Commons = useOutletContext();
	useEffect(()=>{
		axios.get("http://seoulbike-kw.namisnt.com:8082/rest/getMonthlyRankingList")
		.then((res=>{
        ...
        axios.get("http://seoulbike-kw.namisnt.com:8082/rest/service/getMonthlyRanking")
		.then((res=>{
			if(res.data.result === "success") {
				//console.log(res.data);
	...
	<div className={style.content}>
	        <div className={style.my}>
	            <ul className={`${style.tabs} ${style.list}`}>
	            	<Link to='/memberBikeRankWeek'><li className={"link"} data-tab="tab-1">주간</li></Link>
	            	<li className={`tab-link ${style.current}`} data-tab="tab-2">월간</li>
	            </ul>
